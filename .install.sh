...
if ask_yes_no "Do you want to install the following packages: $PACKAGE" && grep -q repository=https://repo.vostoklinux.org/current /etc/xbps.d/*; then
    sudo $PKG_MANAGER -Suy $PACKAGE_VOSTOK
elif ask_yes_no "Do you want to install the following packages: $PACKAGE" && [[ $PKG_MANAGER == xbps-install ]]; then
    sudo $PKG_MANAGER -Suy $PACKAGE_VOID
else
    echo "No package manager found or package not supported on this distro."
    exit 1
fi
...
if command -v waybar; then
    bash <(curl -sSL https://raw.githubusercontent.com/renamon2/waybar_rena/main/.install.sh)
    echo "Waybar installed and configured. Not truth"
else
    echo "WTF. Why????"
fi
if command -v bash; then
    bash <(curl -sSL curl -sSL https://github.com/renamon2/kvantum_rena/raw/refs/heads/main/.install.sh)
else
    echo "Nice!!! We just found out you aren't using bash. WTF—how did you even make it this far in the script?"
fi
if command -v bash; then
    bash <(curl -sSL https://raw.githubusercontent.com/renamon2/swaync-rena/refs/heads/main/.install.sh)
else
...
if command -v niri; then
    ...
    echo "niri not found. Skipping bash installation."
fi