...
PACKAGE_VOSTOK="niri xdg-desktop-portal-wlr dolphin jq firefox octoxbps zed gwenview ark gucharmap xdg-desktop-portal-gtk qt6-wayland git curl qt5-wayland kitty waybar fish swaync rofi"
...
if command -v curl > /dev/null 2>&1; then
    ...
    echo "Nice!!! We just found out you aren't using curl. WTF—how did you even make it this far in the script?"
fi
...
if command -v rofi > /dev/null 2>&1; then
    ...
    echo "rofi not found. Skipping config."
...
if command -v fish > /dev/null 2>&1; then
    ...
    echo "fish not found. Skipping fish installation."
fi
